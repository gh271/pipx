...
def ensure_path(location: Path, *, force: bool, prepend: bool = False, all_shells: bool = False) -> Tuple[bool, bool]:
    ...
    if force or (not in_current_path and not need_shell_restart):
        if prepend:
            path_added = userpath.prepend(location_str, "pipx", all_shells)
        else:
            path_added = userpath.append(location_str, "pipx", all_shells)
        if not path_added:
            print(
                pipx_wrap(
...
def ensure_pipx_paths(force: bool, prepend: bool = False, all_shells: bool = False) -> ExitCode:
    ...
    for bin_path in bin_paths:
        (path_added_current, need_shell_restart_current) = ensure_path(bin_path, prepend=prepend, force=force, all_shells=all_shells)
